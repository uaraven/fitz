...
use fitz_core::analytics::{self, FileAnalysis, Metric, Series, SkipReason};
...
use super::{STATE, operation_targets, set_row_status};
...
pub fn open_analytics_dialog(app: &AppWindow) {
    ...
    spawn_analysis(app.as_weak(), targets, generation);
}

/// Analyze `targets` on a worker thread and show the dialog with the results.
/// A file that can't be read is badged with its error and skipped, but does not
/// abort the batch. A result whose generation is stale (the dialog was closed,
/// or another batch started) is dropped.
fn spawn_analysis(weak: Weak<AppWindow>, targets: Vec<PathBuf>, generation: u64) {
    let _ = weak.upgrade_in_event_loop(|app| {
        app.set_busy(true);
        app.set_stage_text("Analyzing…".into());
    });
    std::thread::spawn(move || {
        let total = targets.len();
        let mut metrics = Vec::new();
        let (mut no_date, mut not_mono, mut failed) = (0usize, 0usize, 0usize);

        for (i, path) in targets.iter().enumerate() {
            let text = format!("Analyzing {} ({}/{})", display_name(path), i + 1, total);
            let _ = weak
                .clone()
                .upgrade_in_event_loop(move |app| app.set_stage_text(text.into()));

            match analytics::analyze_file(path) {
                Ok(FileAnalysis::Analyzed(m)) => metrics.push(m),
                Ok(FileAnalysis::Skipped(SkipReason::NoDateObs)) => no_date += 1,
                Ok(FileAnalysis::Skipped(SkipReason::NotMono)) => not_mono += 1,
                Err(e) => {
                    failed += 1;
                    let (path, msg) = (path.clone(), e.to_string());
        ...
        let _ = weak.upgrade_in_event_loop(move |app| {
            ...
            app.set_analytics_skipped_no_date(no_date as i32);
            app.set_analytics_skipped_not_mono(not_mono as i32);
            STATE.with(|s| s.borrow_mut().analytics = metrics);
            replot(&app);
...
/// Close the dialog: drop the cached metrics and bump the generation so an
/// in-flight batch lands stale and is discarded.
